...
struct event_data {
  ...
  event_data() : fd(-1), buffer(new char[DEFAULT_RING_BUFFER_SIZE]), written(0), stop(false) {}

  ~event_data() { delete[] buffer; }
};
...
int process_messages(processor::RingBuffer<event_data>* ring_buffer) {
  ...
  while (true) {
    ...
    for (int64_t index = prev_sequence + 1; index <= next_sequence; index++) {
      ...
      if (e_data->stop) goto exit_consumer;
...
void accept_handle(int epfd, int sfd) {
  while (true) {
...
void event_loop(int epfd, int sfd, processor::RingBuffer<event_data>* ring_buffer) {
  int n, i;
  int retval;
  ...
  while (true) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if ((current_event.events & EPOLLERR) || (current_event.events & EPOLLHUP) ||
        ...
        // An error has occured on this fd, or the socket is not ready for reading (why were we
        // notified then?).
        ...
        // Closing the descriptor will make epoll remove it from the set of descriptors which are
        // monitored.
        close(current_event.data.fd);
